...
for (let index = 0; index < exemploFor.length; index += 1) {
  const element = exemploFor[index];
  console.log('Exemplo de for, estou no index ', index, ' pegando o valor ', element);
}

// While, usado geralmente quando queremos executar determinada instrução
// ate que determinada condiçãos seja atendida
let exemploWhile = 0;
